...
sbt clean -Dbrowser="${BROWSER:=chrome}" -Denvironment="${ENVIRONMENT:=local}" -Dbrowser.option.headless=true -Dbrowser.usePreviousVersion=true -Dsecurity.assessment=false test testReport
